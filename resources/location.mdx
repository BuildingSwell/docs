...
`DELETE /location/{id}` archives the location and its active descendants. To archive, use DELETE; PATCH accepts `isArchived: false` for restoration, but rejects `isArchived: true`. Project-owned Jobsite and referenced locations have additional lifecycle constraints. A restore targets the addressed record; do not assume it restores every descendant.
