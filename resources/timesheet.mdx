...
| `stageId` | UUID \| null | **Stage Master** id; populated **only** for `build_order_work_session` records |
...
```bash
curl "https://<host>/api/v2/timesheet?type=deliverable_work_session\
...
- `s.name` → the **Stage Master** (joined via the node's `data.stageId`)
- `d.computed.phase` → the **Phase name** (read straight off the joined node)

<Note>
  The `s` join matches `stage.id` against `data->>'stageId'`; the engine casts both sides to `uuid` automatically. Phase/component nodes won't have a Stage Master, so `s` is `null` for those rows — read `computed.phase` for the phase regardless.
</Note>
...
For `build_order_work_session` rows (order-level time), there is no deliverable node — join the Order and Stage Master directly off the timesheet's own keys:
