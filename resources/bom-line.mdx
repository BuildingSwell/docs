...
Supply `itemId` and exactly one of `routeDeliverableId`, `orderDeliverableId`, or `projectId`. A route needs a positive `outputPerUnit` or `quantity`; orders and projects need a positive `quantity`. The name and unit default from the item.
...
| Endpoint | Request |
| --- | --- |
| `POST /bom-line/order/{id}/prepopulate` | `{}` or `{ "routeDeliverableId": "<uuid>" }`; seeds the order BOM. |
