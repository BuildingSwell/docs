...
```http
GET /deliverable?type=order&data.properties.squareFeet:gt=100
GET /deliverable?type=order&select=id,name,data.properties.squareFeet
```

For totals by project, use a grouped query with numeric aggregates:

```http
POST /deliverable/query
Content-Type: application/json

{
  "type": "order",
  "groupBy": "data.projectId",
  "select": [
    "data.projectId",
    { "aggregate": "SUM", "field": "data.properties.squareFeet", "alias": "totalSquareFeet" },
    { "aggregate": "AVG", "field": "data.properties.squareFeet", "alias": "averageSquareFeet" }
  ]
}
```

Use `POST /deliverable/group-and-count` with `groupByField: "data.properties.squareFeet"` when you want counts by value. See [grouping and aggregates](/guides/group-and-count).
