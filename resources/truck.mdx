...
Only `name` is required. Weight fields are pounds; deck dimensions and maximum height are feet. Capacity values must be positive when supplied. Link a delivery with `truckId`; `truck` on the delivery is its legacy free-text label. DELETE archives the truck.
