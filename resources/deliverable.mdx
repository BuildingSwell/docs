...
Central, **generic** resource: one model implements nine domain **concepts** via its `type` field. The raw `deliverable` endpoints below are low-level and do **not** enforce concept rules (valid nesting, required fields, Stage→Master linkage). For concept-aware, validated work — and to learn how the concepts fit together — use the [Domain concepts](/concepts/overview) endpoints.
...
| `stage` | work node | A Department work-station. **Must** link a Stage Master via `data.stageId`. |
