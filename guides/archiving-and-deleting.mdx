...
This holds for every archivable resource, People included. An explicit
top-level `isArchived` in your query replaces the default rather than adding to
it, so a single request returns either active or archived records, never both.
