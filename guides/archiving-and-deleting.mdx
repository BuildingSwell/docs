...
- `archivedAt` is stamped with the current time, and cleared again on restore.
- The record's name or identifier is given an ` - (archived)` suffix so it stops
  colliding with your active records. Restoring strips the suffix, and picks the
  next free variant if something has taken the original name meanwhile.
...
<Note>
  Not every resource is archivable. Work sessions, notes, documents, comments, and
  QC records have no `isArchived` field, so for those the only removal is a delete.
</Note>
...
| [Stage](/resources/stage) | Removes the station. Deliverable stage nodes keep a `data.stageId` that no longer resolves, since that link is JSON rather than a foreign key. |
...
| [Comment](/resources/comment) | Removes the row and its replies. The app instead soft-deletes by setting `deletedAt`. |
| [Document](/resources/document) | Removes the document record. |
| [Note](/resources/note) | Removes the note and its contents. |
...
wipe out a table in one call, by design. Delete records one at a time, or archive
them in bulk.
...
The two you will actually hit are projects, which always own a Jobsite, and QC
template versions that inspection sets still reference. In both cases the answer is
to archive, or to remove the referencing records first.
